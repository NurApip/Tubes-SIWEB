...
@section('admin_content')
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white p-4 rounded-[32px] shadow-sm border border-slate-200 group">
                ...
                <p class="text-[10px] text-slate-400 font-bold uppercase">
                    {{ $f->lokasi }} â€¢ Rp {{ number_format($f->harga_per_jam ?? 0, 0, ',', '.') }}/Jam
                </p>
                ...
                <div class="flex gap-2 mt-5">
                    ...
                    <form action="/admin/fields/{{ $f->lapangan_id }}/delete" method="POST" onsubmit="return confirm('Yakin ingin menghapus lapangan ini?')" class="inline">
                        @csrf
                        @method('DELETE')
            ...
            <form id="modalForm" action="{{ route('admin.fields.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-5">
                    ...
                    <div>
                        ...
                        <input type="file" name="foto" id="fotoInput" required class="w-full bg-slate-50 border-2 border-slate-50 rounded-2xl p-4 outline-none focus:border-blue-400 transition">
                    </div>
                </div>
    ...
    <script>
        ...
        function ensureIsActiveHiddenMatchesCheckbox() {
            ...
            if (hidden && isActiveToggle) hidden.value = isActiveToggle.checked ? '1' : '0';
        }
