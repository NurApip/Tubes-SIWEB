...
@section('admin_content')
    ...
    @if(session('error'))
        <div class="mb-8 p-5 bg-red-50 border border-red-100 rounded-3xl text-red-700 font-black uppercase tracking-wider">{{ session('error') }}</div>
    @endif

    @if($errors->any())
        <div class="mb-8 p-5 bg-red-50 border border-red-100 rounded-3xl text-red-700 font-black uppercase tracking-wider">{{ $errors->first() }}</div>
    @endif
        ...
        <div class="lg:col-span-2 bg-white rounded-[32px] p-8 border border-slate-200 shadow-sm">
            ...
            <div class="bg-white p-8 rounded-[32px] border border-slate-200 shadow-sm">
                ...
                @if($booking->status === 'Pending')
                    <form action="{{ route('admin.bookings.update', $booking->id) }}" method="POST" class="space-y-6" onsubmit="return confirm('Status hanya dapat diubah satu kali. Lanjutkan?');">
                        @csrf
                        <div>
                            <label class="block text-[10px] font-black uppercase text-slate-500 mb-2">Ubah Status</label>
                            <select name="status" required class="w-full bg-slate-50 border border-slate-200 rounded-3xl p-4 text-sm font-black uppercase text-slate-800">
                                <option value="" selected disabled>Pilih status final </option>
                                <option value="Success">Success</option>
                                <option value="Cancelled">Cancelled</option>
                            </select>
                           
                        </div>
                        <button type="submit" class="inline-flex items-center justify-center w-full px-6 py-4 text-[10px] font-black uppercase rounded-3xl bg-blue-600 text-white hover:bg-blue-700 transition">Simpan Status Final</button>
                    </form>
                @else
                    <div class="p-5 bg-slate-50 border border-slate-200 rounded-3xl">
                        <p class="text-sm font-black uppercase text-slate-700">Status sudah final: {{ $booking->status }}</p>
                        <p class="mt-2 text-[10px] font-bold uppercase text-slate-500">Status booking ini telah dikunci dan tidak dapat diubah lagi.</p>
                    </div>
                @endif
            </div>
        </div>
