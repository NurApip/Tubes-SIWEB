...
@if(session('error'))
    <div class="mb-6 bg-red-50 text-red-700 border border-red-200 p-4 rounded-2xl text-sm font-black">
        {{ session('error') }}
    </div>
@endif
        ...
        <tbody>
            ...
            <tr class="border-b last:border-0 hover:bg-slate-50/50 transition">
                ...
                <td class="py-4">
                    ...
                    @elseif($user->isExpiredMember())
                        <span class="bg-red-50 text-red-700 text-[10px] px-3 py-1.5 rounded-xl font-black uppercase tracking-wider border border-red-100">
                            Kedaluwarsa
                        </span>
                    @else
                ...
                <td class="py-4 text-[10px] font-bold uppercase text-slate-500">
                    @if($user->membership_started_at && $user->membership_expires_at)
                        <p>{{ $user->membership_started_at->format('d M Y') }}</p>
                        <p class="mt-1 text-slate-400">s/d {{ $user->membership_expires_at->format('d M Y') }}</p>
                    @elseif($user->isPendingMember() && $user->membership_requested_at)
                        <p class="text-yellow-600">Diajukan {{ $user->membership_requested_at->format('d M Y') }}</p>
                    @else
