...
<body class="bg-gray-100 h-screen flex">
    ...
    <div class="w-1/2 bg-white p-12 flex flex-col justify-center">
        ...
        <form action="/register" method="POST" class="space-y-4">
            @csrf
            <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Nama Lengkap" required class="w-full border p-3 rounded">
            <input type="email" name="email" value="{{ old('email') }}" placeholder="Email" required class="w-full border p-3 rounded">
            <input type="text" name="hp" value="{{ old('hp') }}" placeholder="No. HP" required class="w-full border p-3 rounded">
